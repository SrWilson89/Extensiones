  ...
  function loadEmails() {
    ...
    chrome.runtime.sendMessage({ action: 'getEmails' }, (response) => {
      if (response.success) {
        ...
        // === MODIFICACIÓN CLAVE PARA EVITAR EL CIERRE AUTOMÁTICO DE SESIÓN ===
        // Al recibir un error (ej. 401: Token expirado), solo mostramos un mensaje,
        // pero mantenemos el estado de isAuthenticated = true y NO llamamos a updateUIState()
        // para que el botón de 'Cerrar Sesión' y el indicador 'Conectado' se mantengan.
        emailList.innerHTML = `<p id="status-message">Error al cargar correos: ${response.error}. Por favor, haz clic en Actualizar o prueba a Cerrar Sesión y volver a conectar.</p>`;
        // Líneas originales que se REMUEVEN/COMENTAN:
        // isAuthenticated = false; 
        // updateUIState();
        // ======================================================================
      }
